...
func mainCassa(u string) {
	// Generic initialization of gocql
	c := gocql.NewCluster(u)
	// c := gocql.NewCluster("172.17.0.1")
...
func createKeyspace(s *gocql.Session) {
	...
	keyspace := make(map[string]gocqltable.Keyspace)
	for i := 0; i < 20; i++ {
		keyspaceName := "gocqltable_test" + fmt.Sprint(i)
		// Now we're ready to create our first keyspace. We start by getting a keyspace object
		temp := gocqltable.NewKeyspace(keyspaceName)
		temp.Drop()
		keyspace[keyspaceName] = temp

		// Now lets create that in the database using the simple strategy and durable writes (true)
		err := keyspace[keyspaceName].Create(map[string]interface{}{
			"class":              "SimpleStrategy",
			"replication_factor": 1,
		}, true)
		if err != nil { // If something went wrong we print the error and quit.
			fmt.Println("Keyspace created error ", keyspaceName, err)
		}
		fmt.Println("Keyspace created ", keyspaceName)
		defer keyspace[keyspaceName].Drop()

		// TABLES
		createTable(keyspace[keyspaceName])
		fmt.Println("Keyspace finish ", keyspaceName)
	}
...
func createTable(keyspace gocqltable.Keyspace) {
	for i := 0; i < 10; i++ {
		tableName := "user_" + fmt.Sprint(i)
		...
		for j := 0; j < 100; j++ {
			uName := fmt.Sprint(j) + "@example.com"
			q := fmt.Sprintf("INSERT INTO %s.%s (email, password, active, created) VALUES (?, ?, ?, ?)", keyspace.Name(), tableName)
			err = userTable.Query(q, uName, "123456", true, time.Now().UTC()).Exec()
			if err != nil {
				log.Fatalln(err)
			}
		...
		q := fmt.Sprintf("SELECT * FROM %s.%s", keyspace.Name(), tableName)
		rows, err := userTable.Query(q).FetchRow()
